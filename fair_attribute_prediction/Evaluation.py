...
def evaluate(
    ...
    prediction_count = attribute_targets.shape[0]
    if state is None:
        state = EvaluationState()
        state.loss_term_totals = defaultdict(float)
        state.correct_prediction_counts = zeros(
            attribute_targets.shape[1], device=get_device()
        )
    ...
    state.correct_prediction_counts += attribute_predictions.eq(attribute_targets).sum(dim=0)
